...
(SQLite). Only non-secret defaults, the admin password and the shared API
token come from the environment (set by run.sh from the add-on options).
"""
import os
import secrets
...
# Shared bearer token every public /v1 call must present. Set/generated by
# run.sh (add-on option `api_token`, else auto-generated and persisted to
# /data). The iOS app and the local bridge send it as `Authorization: Bearer`.
# When empty the /v1 API fails closed (503) rather than accepting unauthenticated
# control of pushes / the disarm gate.
API_TOKEN = os.environ.get("APEX_API_TOKEN", "").strip()

# Mark the admin session cookie Secure (HTTPS-only). Enable this when the admin
# page is reached only over HTTPS (e.g. through the tunnel); leave off if you use
# the local "OPEN WEB UI" button, which is plain HTTP.
COOKIE_SECURE = os.environ.get("APEX_COOKIE_SECURE", "").strip().lower() in ("1", "true", "yes", "on")
